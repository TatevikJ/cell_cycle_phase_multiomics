...
#SBATCH -c 4
#SBATCH -p long
#SBATCH --output=log/pipeline_v3-slurm-%j.out   # %j will be replaced with the Job ID
...
awk -F, 'NR>1 {print $1,$2,$3,$4}' $INPUT_PATHS_CSV | while read SAMPLE PEAKS_PATH FRAGMENTS_PATH BARCODES_PATH; do
	...
	awk '$1 == $6 && $4 == $9 && ($2 < $7 || ($2 == $7 && $3 < $8))' - \
	    > ${OUTPUT_DIR}/${SAMPLE}/atac_overlapping_fragment_pairs.bed
    
    awk '{print $1, $2, $3, $4, $5; print $6, $7, $8, $9, $10}' OFS="\t" ${OUTPUT_DIR}/${SAMPLE}/atac_overlapping_fragment_pairs.bed | \
...
awk -F, 'NR>1 {print $1,$2,$3,$4}' $INPUT_PATHS_CSV | while read SAMPLE PEAKS_PATH FRAGMENTS_PATH BARCODES_PATH; do
    ...
    bedtools intersect \
             ...
             -sorted | \
	awk '$1 == $6 && $4 == $9 && ($2 < $7 || ($2 == $7 && $3 < $8))' - \
	    > ${OUTPUT_DIR}/${SAMPLE}/atac_overlapping_fragment_pairs_exclude_peak_regions.bed
    
    awk '{print $1, $2, $3, $4, $5; print $6, $7, $8, $9, $10}' OFS="\t" ${OUTPUT_DIR}/${SAMPLE}/atac_overlapping_fragment_pairs_exclude_peak_regions.bed | \
	sort -k1,1 -k2,2n -k3,3n -u \
...
awk -F, 'NR>1 {print $1,$2,$3,$4}' $INPUT_PATHS_CSV | while read SAMPLE PEAKS_PATH FRAGMENTS_PATH BARCODES_PATH; do
    ...
    bedtools intersect \
             ...
             -sorted | \
	awk '$1 == $6 && $4 == $9 && ($2 < $7 || ($2 == $7 && $3 < $8))' - \
	    > ${OUTPUT_DIR}/${SAMPLE}/atac_overlapping_fragment_pairs_exclude_peak_gene_regions.bed
    
    awk '{print $1, $2, $3, $4, $5; print $6, $7, $8, $9, $10}' OFS="\t" ${OUTPUT_DIR}/${SAMPLE}/atac_overlapping_fragment_pairs_exclude_peak_gene_regions.bed | \
	sort -k1,1 -k2,2n -k3,3n -u \
...
awk -F, 'NR>1 {print $1,$2,$3,$4}' $INPUT_PATHS_CSV | while read SAMPLE PEAKS_PATH FRAGMENTS_PATH BARCODES_PATH; do
    ...
    bedtools intersect \
             ...
             -sorted | \
	awk '$1 == $6 && $4 == $9 && ($2 < $7 || ($2 == $7 && $3 < $8))' - \
	    > ${OUTPUT_DIR}/${SAMPLE}/atac_overlapping_fragment_pairs_only_peak_regions.bed
    
    awk '{print $1, $2, $3, $4, $5; print $6, $7, $8, $9, $10}' OFS="\t" ${OUTPUT_DIR}/${SAMPLE}/atac_overlapping_fragment_pairs_only_peak_regions.bed | \
	sort -k1,1 -k2,2n -k3,3n -u \
